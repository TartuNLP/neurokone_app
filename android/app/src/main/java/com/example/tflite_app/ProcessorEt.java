...
class ProcessorEt {
    ...
    public List<String> splitSentences(String remainingText) {
        ...
        while ((matcher = sentencesSplit.matcher(remainingText)).find(currentSentId)) {
            ...
            sentences
                    .add(sentence.substring(currentCharId).replaceAll(sentenceStrip, "") + '.');
        }
        return sentences;
    }
    ...
    public int[] textToIds(String text) {
        ...
        while (text!= null && text.length() > 0) {
            ...
            if (!m.find()) {
                sequence.addAll(symbolsToSequence(cleanTextForEstonian(text)));
                break;
            }
            sequence.addAll(symbolsToSequence(cleanTextForEstonian(m.group(1))));
